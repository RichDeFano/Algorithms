...
public class treeNode {
    ...
    public treeNode predecessor( int x ) {
        ...
        if (this.iData > x)                     //If the key is smaller than this key, go to the left subtree.
            {return this.left.predecessor(x);}
        
        if (this.iData == x)                    //If the key is present at this node
            {
                if (this.left != nullnode)      //The predecessor will be the max value in the left subtree
                    {return this.left;}
                else if (this.right != nullnode)       //Or the right if it does not have a left.
                    {return this.right;}
                else
                {return this;}
            }
    ...
    public treeNode successor( int x ) {
        ...
        if (this.iData > x)                     //If the key is smaller than this key, go to the left subtree.
            {return this.right.predecessor(x);}
        
        if (this.iData == x)                    //If the key is present at this node
            {
                if (this.right != nullnode)      //The predecessor will be the max value in the left subtree
                    {return this.left;}
                else if (this.left != nullnode)       //Or the right if it does not have a left.
                    {return this.left;}
                else
                {return this;}
            }
    ...
    public int inOrder(int i, int[] res){
        //I is the first position available in res
        //Returns the next available position in res[] after it adds everything
        //Add all numbers in the subtree rooted by the current (this) node
        
        //In order traversal:
            //Check if the current node is empty or null. Otherwise 
                if (this == nullnode ) {System.out.println("Node is Empty.");}
            //Traverse the left subtree by cecursively calling the in order function
                if (this.left != nullnode)
                {i = this.left.inOrder(i+1, res);}
            //Display the data part of the current node
                //System.out.println("Node = " + this.iData);
                res[i] = this.iData;
               //i = i+1;
            //Traverse the right subtree by recursively calling the in order function
                if (this.right != nullnode)
                {i = this.right.inOrder(i+1, res);}
